    ...
    <div className="flex h-screen bg-surface overflow-hidden">
      ...
      <main className="flex-grow overflow-y-auto p-4 md:p-8 pb-32 md:pb-8">
        <div className="max-w-7xl mx-auto">
          {/* SaaS Subscription Status Warning Banner */}
          {user && user.planStatus !== 'active' && user.planStatus !== 'trialling' && (
            <motion.div 
              initial={{ opacity: 0, y: -10 }}
              animate={{ opacity: 1, y: 0 }}
              className="mb-6 bg-error-container text-on-error-container border border-error/20 p-4 rounded-2xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-sm"
            >
              <div className="flex gap-3 items-start">
                <span className="material-symbols-outlined text-error text-2xl">workspace_premium</span>
                <div>
                  <h4 className="font-bold text-sm tracking-tight text-error">Assinatura Inativa ou Expirada ({user.planStatus})</h4>
                  <p className="text-xs opacity-90 mt-0.5">As permissões de gravação e leitura no Firestore estão bloqueadas para a conta <b>{user.name}</b> pelo Middleware de Assinatura do SaaS comercial.</p>
                </div>
              </div>
              <button 
                onClick={() => setIsProfileOpen(true)}
                className="bg-error text-on-error hover:opacity-90 transition-opacity text-xs font-bold px-3 py-1.5 rounded-xl flex items-center gap-1 shrink-0 cursor-pointer shadow-sm"
              >
