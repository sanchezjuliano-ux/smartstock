...
export default function LoginView({ onLogin }: { onLogin: (profile: any) => void }) {
  ...
  const handleCreateProfile = (e: React.FormEvent) => {
    ...
    if (hasAdmin && newRole === 'admin') {
      alert('Perfil de Administrador criado com sucesso!');
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      const finalProfile = {
        ...loggedProfile,
        role: isAdminOverride ? 'admin' : loggedProfile.role
      };
      
      if (isAdminOverride && loggedProfile.role !== 'admin') {
        setProfiles(prev => prev.map(p => p.name === loggedProfile.name ? { ...p, role: 'admin' } : p));
      }
          ...
          <div className="p-8 md:p-12 animate-in slide-in-from-bottom-4 fade-in duration-300">
            <div className="flex flex-col items-center max-w-sm mx-auto">
              <form className="w-full space-y-6" onSubmit={handleCreateProfile}>
                ...
                {hasAdmin && newRole === 'admin' && (
                  <div className="space-y-1 animate-in fade-in slide-in-from-top-2 bg-secondary/10 p-4 rounded-lg border border-secondary/20 text-left">
                    <p className="text-xs text-secondary leading-relaxed font-semibold">
                      ✓ Privilégio de Administrador ativado para este perfil. Ele terá acesso total e irrestrito ao gerenciamento do sistema.
                    </p>
                  </div>
                )}
          ...
          <div className="p-8 md:p-12 animate-in slide-in-from-bottom-4 fade-in duration-300">
            <div className="flex flex-col items-center max-w-sm mx-auto">
              ...
              <form className="w-full space-y-6" onSubmit={handleSubmit}>
                ...
                {/* Opção de privilégio de Administrador */}
                <div className="bg-surface-container-low border border-outline-variant/60 rounded-xl p-3.5 flex items-start gap-3 transition-all hover:bg-surface-container-high/40 animate-in fade-in duration-200 text-left">
                  <div className="flex items-center h-5">
                    <input
                      ...
                      className="h-4.5 w-4.5 rounded border-outline text-secondary focus:ring-secondary/50 focus:ring-2 cursor-pointer"
                    />
                  </div>
                  <div className="text-left">
                    <label htmlFor="admin-override" className="text-xs font-bold text-on-surface uppercase tracking-wider block cursor-pointer select-none">
                      Privilégio de Administrador
                    </label>
                    <p className="text-[10px] text-on-surface-variant leading-normal mt-0.5">
                      Ative esta opção para fazer login com privilégios completos de administrador nesta sessão.
                    </p>
                  </div>
                </div>
